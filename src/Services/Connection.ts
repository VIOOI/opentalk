...
import { AdsServiceLive } from "./Ads.js";
...
export class ConnectionService extends Context.Tag("ConnectionService")<
  ConnectionService,
  {
    ...
    getCompanion: (
      context: Types.Context,
    ) => Effect.Effect<User, UserIsNotConnection | UserNotFoundError>;
    disconnect: (
      self: Types.Context,
    ) => Effect.Effect<void, UserIsNotConnection | UserNotFoundError>;
...
export const ConnectionServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return {
      connect: (context: Types.Context, self: Queue, that: Queue) =>
        Effect.all(
          {
          ...
          Effect.tap(({ that }) => context.session.connect == that),
      ...
      getCompanion: (context: Types.Context) =>
        Effect.promise(() =>
      ...
      isInConnection: (self: Types.Context) =>
        Effect.promise(() => Redis.get(`connect:${self.from!.username!}`)).pipe(
          Effect.filterOrFail(isNotNull, () => new UserIsNotConnection()),
